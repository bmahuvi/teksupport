<div class="h-[calc(100vh-2rem)] w-full flex flex-col relative bg-white" x-data="{ showMobileDetails: false }">
    ...
    @if (session()->has('success') || session()->has('error'))
        <div class="absolute top-4 left-1/2 transform -translate-x-1/2 z-50 w-full max-w-md px-4 pointer-events-none">
            <div
                class="{{ session()->has('success') ? 'bg-green-50 text-green-800 border-green-200' : 'bg-red-50 text-red-800 border-red-200' }} border shadow-lg rounded-lg p-4 flex items-center justify-between animate-in slide-in-from-top-4 fade-in duration-300 pointer-events-auto">
    ...
    @if($selectedTicket)
        ...
        <div class="flex flex-1 w-full h-full overflow-hidden" wire:key="view-ticket-{{ $selectedTicket->id }}">
            ...
            >
                ...
                <div class="flex-1 overflow-y-auto p-5 space-y-8">
                    ...
                    <div class="bg-gray-50 rounded-xl border border-gray-200 p-4">
                        ...
                        <span
                            class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-opacity-10 border"
                            style="background-color: {{ $selectedTicket->status->color }}10; color: {{ $selectedTicket->status->color }}; border-color: {{ $selectedTicket->status->color }}20;">
                            <span class="w-2 h-2 rounded-full mr-2"
                                  style="background-color: {{ $selectedTicket->status->color }}"></span>
            ...
            <div class="flex-1 flex flex-col min-w-0 bg-white relative h-full">
                <div
                    class="h-16 flex-none bg-white border-b border-gray-200 flex items-center justify-between px-4 lg:px-6 z-20 shadow-sm">
                    <div class="flex items-center gap-3 min-w-0">
                        <button wire:click="backToList"
                                class="p-2 -ml-2 text-gray-500 hover:bg-gray-100 rounded-full transition-colors focus:outline-none"
                                title="{{ __('creators-ticketing::resources.frontend.back_to_list') }}">
                    ...
                    <button @click="showMobileDetails = !showMobileDetails"
                            class="lg:hidden p-2 text-gray-500 hover:bg-gray-100 rounded-full focus:outline-none">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                @if(auth()->check())
                                    ...
                                    >
                                        ...
                                        @if(count($userTickets) > 0)
                                            <span
                                                class="ml-2 px-2 py-0.5 bg-blue-100 text-blue-700 text-xs font-bold rounded-full">{{ count($userTickets) }}</span>
                                        @endif
                                    </button>
                                @endif
                            </nav>
                                ...
                                <div wire:key="submit-form-container">
                                    ...
                                    <form wire:submit.prevent="submit" class="max-w-4xl mx-auto">
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            ...
                                            {{-- Department & Category --}}
                                            <div
                                                class="col-span-1 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-gray-50 rounded-xl border border-gray-200">
                                                <div
                                                    class="{{ count($available_forms) > 1 ? 'col-span-1' : 'col-span-1 md:col-span-2 max-w-md mx-auto w-full' }}">
                                                    <label class="block text-sm font-semibold text-gray-900 mb-2">
                                                        {{ __('creators-ticketing::resources.frontend.select_department') }}
                                                        <span class="text-red-500">*</span>
                                                    </label>
                                                    <div class="relative">
                                                        <select wire:model.live="department_id"
                                                                class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm py-2.5 pl-3 pr-10 transition-shadow appearance-none bg-white">
                                                            <option
                                                                value="">{{ __('creators-ticketing::resources.frontend.choose_department') }}</option>
                                                ...
                                                @if(count($available_forms) > 1)
                                                    <div class="col-span-1 animate-in fade-in slide-in-from-left-2">
                                                        <label class="block text-sm font-semibold text-gray-900 mb-2">
                                                            {{ __('creators-ticketing::resources.frontend.category_label') }}
                                                            <span class="text-red-500">*</span>
                                                        </label>
                                                        <div class="relative">
                                                            <select wire:model.live="form_id"
                                                                    class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm py-2.5 pl-3 pr-10 transition-shadow appearance-none bg-white">
                                                                <option
                                                                    value="">{{ __('creators-ticketing::resources.frontend.select_option') }}</option>
                                            ...
                                            <div wire:loading wire:target="department_id, form_id"
                                                 class="col-span-1 md:col-span-2 py-8 text-center">
                                                <div
                                                    class="inline-flex items-center gap-2 text-gray-500 bg-white px-4 py-2 rounded-full shadow-sm border">
                                                    ...
                                                    <span
                                                        class="text-sm font-medium">{{ __('creators-ticketing::resources.frontend.loading_fields') }}</span>
                                                </div>
                                            </div>
                                                        ...
                                                        @switch($field['type'])
                                                            ...
                                                            @case('radio')
                                                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-1">
                                                                    @foreach($field['options'] ?? [] as $k => $v)
                                                                        <label
                                                                            class="relative flex items-start px-4 py-3 rounded-lg border border-gray-200 cursor-pointer hover:bg-blue-50 hover:border-blue-200 transition-all duration-200 focus-within:ring-2 focus-within:ring-blue-500 focus-within:ring-offset-2">
                                                                            <input type="radio"
                                                                                   wire:model="custom_fields.{{ $field['name'] }}"
                                                                                   value="{{ $k }}"
                                                                                   class="mt-0.5 h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500 flex-shrink-0">
                                                                            <span
                                                                                class="ml-3 block text-sm font-medium text-gray-700 leading-snug">
                                                                                {{ $v }}
                                                                            </span>
                                                                        </label>
                                                                    @endforeach
                                                            ...
                                                            @case('file_multiple')
                                                                <div class="w-full" x-data="{ isUploading: false }">
                                                                    ...
                                                                    <label
                                                                        class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-xl cursor-pointer bg-gray-50 hover:bg-blue-50 hover:border-blue-400 transition-all duration-200 group relative overflow-hidden">
                                                                        <div
                                                                            ...
                                                                            <p class="text-xs text-gray-400 mt-2">
                                                                                @if(!empty($mimes[1]))
                                                                                    <span
                                                                                        class="uppercase">{{ str_replace(',', ', ', $mimes[1]) }}</span>
                                                                                @endif
                                                                                ...
                                                                                @if($file)
                                                                                    <li class="flex items-center justify-between p-2 bg-white border border-gray-200 rounded-lg shadow-sm">
                                                                                        <div
                                                                                            ...
                                                                                            @else
                                                                                                <div
                                                                                                    class="w-8 h-8 rounded bg-gray-100 flex items-center justify-center text-gray-400">
                                                                                                    <svg class="w-4 h-4"
                                                                                                         fill="none"
                                                                                                         stroke="currentColor"
                                                                                                         viewBox="0 0 24 24">
                                                                                            ...
                                                                                            <div class="flex-1 min-w-0">
                                                                                                <span
                                                                                                    class="text-xs text-gray-600 truncate block">{{ $file->getClientOriginalName() }}</span>
                                                                                                <span
                                                                                                    class="text-xs text-gray-400">{{ number_format($file->getSize() / 1024, 1) }} KB</span>
                                                                                            </div>
                                                                                        </div>
                                                                                        <button type="button"
                                                                                                wire:click="removeFile('{{ $field['name'] }}', {{ $isMultiple ? $index : 'null' }})"
                                                                    ...
                                                                    <div x-show="isUploading" style="display: none;"
                                                                         class="w-full mt-2">
                                                                        <div
                                                                            class="flex items-center gap-2 text-sm text-blue-600 bg-blue-50 py-1 px-3 rounded-md">
                                                                            ...
                                                                            <span
                                                                                class="font-medium">{{ __('creators-ticketing::resources.frontend.uploading') }}</span>
                                                                        </div>
                                                                    </div>
                                                            ...
                                                            @default
                                                                ...
                                                                >
                                                        @endswitch
                                                        @error("custom_fields.{$field['name']}") <p
                                                            class="mt-1 text-sm text-red-600 animate-pulse">{{ $message }}</p> @enderror
                                                        @error("custom_fields.{$field['name']}.*") <p
                                                            class="mt-1 text-sm text-red-600 animate-pulse">{{ $message }}</p> @enderror
                                                    </div>
                                                @endforeach
                                            ...
                                            @elseif($department_id && ($form_id || count($available_forms) <= 1))
                                                <div
                                                    class="col-span-1 md:col-span-2 text-center py-16 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
                                ...
                                <div wire:key="ticket-list-container">
                                        ...
                                        <div
                                            ...
                                            <ul class="divide-y divide-gray-100">
                                                @foreach($userTickets as $ticket)
                                                    <li wire:click="viewTicket({{ $ticket->id }})"
                                                        class="hover:bg-gray-50 transition duration-150 ease-in-out cursor-pointer group">
                                                        <div class="px-5 py-5">
                                                            <div class="flex items-center justify-between mb-2">
                                                                <div class="flex items-center gap-3 min-w-0">
                                                                    ...
                                                                    @if($ticket->publicReplies->where('is_seen', false)->where('user_id', '!=', auth()->id())->count() > 0)
                                                                        <span
                                                                            class="flex-shrink-0 inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-red-100 text-red-800 animate-pulse">
                                                                ...
                                                                <div class="flex-shrink-0 ml-2">
                                                                    <span
                                                                        ...
                                                                        style="background-color: {{ $ticket->status->color }}10; color: {{ $ticket->status->color }}; border-color: {{ $ticket->status->color }}20;">
                                                                        {{ $ticket->status->name }}
                                                                    </span>
                                                                </div>
                                                            </div>
                                                            <div
                                                                ...
                                                                <div class="flex items-center gap-2">
                                                                    <span
                                                                        class="font-mono bg-gray-100 px-1.5 py-0.5 rounded text-gray-600">#{{ $ticket->ticket_uid }}</span>
                                                                    <span>&bull;</span>
                                                                    <span
                                                                        class="font-medium text-gray-600">{{ $ticket->department->name }}</span>
                                                                </div>
                                    ...
                                    @else
                                        <div class="text-center py-20">
                                            <div
                                                class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gray-50 mb-6 border border-gray-100">
                                            ...
                                            <div class="mt-8">
                                                <button wire:click="$set('showForm', true)" type="button"
                                                        class="inline-flex items-center px-6 py-3 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors">
