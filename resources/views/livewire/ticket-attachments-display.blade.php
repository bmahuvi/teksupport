@php
    ...
    foreach ($files as $index => $file) {
        ...
        if (\Illuminate\Support\Facades\Route::has('attachment')) {
            $url = route('attachment', ['ticketId' => $ticketId, 'filename' => $filename]);
        } else {
...
>
    ...
    <div class="flex flex-wrap gap-3 items-start">
        @foreach($processedFiles as $file)
            ...
            @if($file['isImage'])
                <div
                    class="attachment-thumb-container rounded-lg overflow-hidden border border-gray-200 bg-gray-50 flex items-center justify-center cursor-pointer transition-shadow hover:shadow-lg"
            ...
            @else
                <a href="{{ $safeUrl }}"
                   ...
                   title="{{ $safeName }}">
                    ...
                    <span
                        class="block w-full text-[10px] text-gray-600 text-center leading-tight overflow-hidden whitespace-nowrap text-ellipsis">
                        {{ $safeName }}
                    </span>
                </a>
            @endif
    ...
    @if(!$hasFiles)
        <div class="text-sm italic text-gray-400 mt-2">{{ 'No file attached' }}</div>
    @endif
    ...
    <template x-teleport="body">
        ...
        >
            ...
            <template x-if="images.length > 1">
                <div
                    style="position:absolute;bottom:2rem;left:50%;transform:translateX(-50%);background:rgba(0,0,0,0.7);color:white;padding:0.5rem 1rem;border-radius:2rem;font-size:0.875rem;backdrop-filter:blur(10px);border:1px solid rgba(255,255,255,0.2);z-index:10000;">
